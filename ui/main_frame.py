...
class MainAppFrame(ttk.Frame):
    ...
    def create_main_layout(self):
        ...
        if self.is_admin:
            ttk.Button(right_buttons_frame, text="👤 İstifadəçi İdarəetməsi", command=self.open_user_management).pack(side='left', padx=2)
            ttk.Button(right_buttons_frame, text="🐞 Xəta Jurnalı", command=self.open_error_viewer).pack(side='left', padx=2)
    ...
    def setup_left_panel(self):
        ...
        employee_frame = ttk.LabelFrame(self.left_frame, text="İşçilər")
    ...
    def _create_vacation_panel(self):
        ...
        self.panel_save_button = ttk.Button(self.vacation_form_panel, text="Sorğu Göndər", command=self._save_vacation_from_panel)
        self.panel_save_button.pack(side='bottom', fill='x', ipady=5, pady=(15, 0))
        
        form_body = ttk.Frame(self.vacation_form_panel, style="Card.TFrame")
    ...
    def _animate_panel(self):
        ...
        start_relx, end_relx = (1.0, 1.0 - 0.45)
        step = 0.05
    ...
    def _save_vacation_from_panel(self):
        ...
        if not start_date_obj or not end_date_obj: messagebox.showerror("Xəta", "Tarixlər seçilməlidir.", parent=self); return
        if end_date_obj < start_date_obj: messagebox.showerror("Xəta", "Bitmə tarixi başlanğıcdan əvvəl ola bilməz.", parent=self); return
