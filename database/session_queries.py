...
def get_all_active_non_admin_user_ids():
    ...
    try:
        with conn.cursor() as cur:
            cur.execute("""
                SELECT s.user_id FROM active_sessions s
...
def add_user_session(user_id, ip_address):
    ...
    try:
        with conn.cursor() as cur:
            cur.execute("INSERT INTO active_sessions (session_id, user_id, ip_address) VALUES (%s, %s, %s)", (str(session_id), user_id, ip_address))
            cur.execute("INSERT INTO login_history (user_id) VALUES (%s) RETURNING id", (user_id,))
            result = cur.fetchone()
            if result: history_id = result[0]
            conn.commit()
            return str(session_id), history_id
    except psycopg2.Error as e:
        messagebox.showerror("Baza Xətası", f"Sessiya yaradılarkən xəta baş verdi:\n{e}"); conn.rollback(); return None, None
    finally:
        if conn: conn.close()
...
def get_active_session_counts():
    ...
    try:
        with conn.cursor() as cur:
            ...
            for user_id, count in cur.fetchall(): counts[user_id] = count
    except psycopg2.Error as e:
...
def force_remove_sessions_by_user_id(user_ids):
    ...
    try:
        with conn.cursor() as cur:
            ...
            cur.execute("UPDATE login_history SET logout_time = CURRENT_TIMESTAMP WHERE user_id IN %s AND logout_time IS NULL", (ids_tuple,))
            cur.execute("DELETE FROM active_sessions WHERE user_id IN %s", (ids_tuple,))
            conn.commit()
        return True
    except psycopg2.Error as e:
        messagebox.showerror("Baza Xətası", f"Sessiyalar silinərkən xəta baş verdi:\n{e}"); conn.rollback(); return False
    finally:
        if conn: conn.close()
