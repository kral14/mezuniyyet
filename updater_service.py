# updater_service.py (Setup endirən versiya)
...
class UpdaterService:
    ...
    def _run_update_task(self):
        try:
            ...
            # Adında 'setup' olan .exe faylını axtarırıq
            ...
            response = requests.get(asset_url, stream=True, timeout=180) # Vaxtı artırdıq
            response.raise_for_status()
            ...
            # Endirilmiş setup faylını işə salırıq
            subprocess.Popen([save_path])
            # və əsas proqramı dərhal bağlayırıq
            sys.exit()
