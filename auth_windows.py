# auth_windows.py ("Məni xatırla" ilə)
...
class LoginFrame(ttk.Frame):
    def __init__(self, parent, login_callback, register_callback, last_username=""):
        ...
        style = ttk.Style(self); style.configure('TFrame'); style.configure('TLabel', font=('Helvetica', 11)); style.configure('TButton', font=('Helvetica', 10, 'bold'))
        self.configure(style='TFrame')
        container = ttk.Frame(self, style='TFrame'); container.pack(expand=True)
        
        title_font = tk.font.Font(family="Helvetica", size=18, weight="bold")
        ttk.Label(container, text="Sistemə Giriş", font=title_font, foreground="#333").pack(pady=(0, 25))

        self.username = tk.StringVar(value=last_username) # Son istifadəçi adını göstəririk
        self.password = tk.StringVar()
        self.remember_me = tk.BooleanVar(value=bool(last_username)) # Əgər ad varsa, seçili olsun
        ...
        username_entry.pack(padx=10, pady=(0,10)); username_entry.focus()
        ttk.Label(container, text="Şifrə:").pack(padx=10, pady=(10,0), anchor='w')
        ttk.Entry(container, textvariable=self.password, show="*", width=35, font=('Helvetica', 11)).pack(padx=10, pady=(0,10))
        
        ttk.Checkbutton(container, text="Məni xatırla", variable=self.remember_me).pack(pady=10)

        button_frame = ttk.Frame(container, style='TFrame'); button_frame.pack(pady=15)
...
# RegisterFrame dəyişməz qala bilər, ancaq tamlıq üçün onu da verirəm
class RegisterFrame(ttk.Frame):
    def __init__(self, parent, register_callback, back_to_login_callback):
        super().__init__(parent, padding="20")
        self.register_callback = register_callback; self.back_to_login_callback = back_to_login_callback
        style = ttk.Style(self); style.configure('TFrame'); style.configure('TLabel', font=('Helvetica', 11)); style.configure('TButton', font=('Helvetica', 10, 'bold'))
        self.configure(style='TFrame')
        container = ttk.Frame(self, style='TFrame'); container.pack(expand=True)
        title_font = tk.font.Font(family="Helvetica", size=18, weight="bold")
        ttk.Label(container, text="Yeni İşçi Qeydiyyatı", font=title_font, foreground="#333").pack(pady=(0, 20))
        self.name = tk.StringVar(); self.username = tk.StringVar(); self.password = tk.StringVar(); self.confirm_password = tk.StringVar()
